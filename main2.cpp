...
Obj mob[3] = {
	...
	{(char*)"◆", 3, 8, 60, rand()%30+1, rand()%10+1, rand()%30+1 }
};
int count;
int php = player.hp;
		...
		if (turn % 2 == 0) { // player turn
			...
			if (player.luk >= rand() % 50 + 1) {
				...
				if (player.luk >= rand() % 50 + 1) { // 치명타, 방어력 무시
					php -= mob.atk;
					MoveCursor(2, 25, "");
					printf("치명타! %d만큼의 대미지를 입었습니다.  남은 체력: %d                                                 ", mob.atk, php);
				}
				else {
					php -= mob.atk - (mob.atk * player.def / 100); // 방어력% 만큼 공격 대미지 감쇠
					MoveCursor(2, 25, "");
					printf("유효타! %d만큼의 대미지를 입었습니다.  남은 체력: %d                                                 ", mob.atk - (mob.atk * player.def / 100), php);
				}
			}
		}
